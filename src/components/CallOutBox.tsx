...
const typeConfig = {
  critical: { border: "border-l-[#ef4444]", bg: "bg-[#fee2e2]", icon: AlertTriangle },
  tip: { border: "border-l-[#3b82f6]", bg: "bg-[#dbeafe]", icon: Lightbulb },
  goal: { border: "border-l-[#10b981]", bg: "bg-[#d1fae5]", icon: Target },
  safety: { border: "border-l-[#f59e0b]", bg: "bg-[#fef3c7]", icon: Shield },
};
...
export default function CallOutBox({ type = "tip", title, children }: Props) {
  ...
  return (
    <div className={`flex gap-3 p-4 rounded-lg border-l-4 ${config.border} ${config.bg} no-break`}>
