...
const offlineRows = [
  ...
  { feature: "توافق الأجهزة", detail: "Android و iOS عبر المتصفح (PWA)" },
];
...
const migrationRows = [
  ...
  { src: "معرفة الفنيين", data: "تاريخ الأعطال، خبرات", status: "غير موثقة", options: "ورش عمل توثيق قبل الترحيل" },
...
const migrationPlanRows = [
  ...
  { phase: "5 — التاريخي", content: "سجلات سابقة للتحليل", method: "إدخال تدريجي حسب الأهمية", duration: "مستمر" },
];
...
const riskRows = [
  ...
  { risk: "فقدان البيانات (حذف غير مقصود)", prob: "منخفض", impact: "عالٍ جداً", mitigation: "نسخ احتياطي يومي + استضافة سحابية" },
  { risk: "هجوم إلكتروني (Ransomware)", prob: "منخفض", impact: "عالي", mitigation: "تحديثات أمنية + firewall + تدريب" },
  { risk: "تسريب بيانات الأصول", prob: "منخفض", impact: "متوسط", mitigation: "صلاحيات وصول محدودة + تشفير" },
  { risk: "انقطاع الخدمة (DDoS)", prob: "متوسط", impact: "متوسط", mitigation: "خطة استرداد + استضافة احتياطية" },
];
const securityCols = [{ key: "area", label: "المجال" }, { key: "action", label: "الإجراء" }];
const securityRows = [
  { area: "المصادقة", action: "اسم مستخدم + كلمة مرور قوية (8 أحرف + رمز + حرف كبير)" },
  { area: "المصادقة الثنائية", action: "اختيارية للحسابات الحساسة (مدير الصيانة، الإدارة العليا)" },
  { area: "الصلاحيات", action: "كل مستخدم يرى فقط ما يحتاجه — فني يرى أوامر عمله فقط" },
  ...
  { area: "سجل التدقيق (Audit Log)", action: "كل حركة مسجلة: من فعل ماذا ومتى" },
  { area: "إدارة الجلسات", action: "تسجيل خروج تلقائي بعد 30 دقيقة من inactivity" },
...
const bcRows = [
  ...
  { scenario: "عطل في قاعدة البيانات", action: "استرجاع آخر نسخة احتياطية صالحة (خسارة قصوى: يوم عمل واحد)", resp: "فريق الدعم التقني" },
  ...
  return (
    <section className="space-y-6 print:space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        ...
        <h4 className="text-md font-bold text-slate-700 mb-3">تنظيم أوامر العمل</h4>
        <DataTable columns={woCompareCols} rows={woCompareRows} />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">إدارة الصيانة الوقائية</h4>
        <DataTable columns={pmBenefitsCols} rows={pmBenefitsRows} />
        <CallOutBox type="tip" title="رفع كفاءة اتخاذ القرار">
          بدلا من الاعتماد على التقديرات الشخصية، يوفر النظام تقارير دورية آلية، وإحصائيات دقيقة عن الأداء والتكاليف، ومؤشرات أداء تساعد الإدارة على اتخاذ قرارات مبنية على بيانات حقيقية.
        </CallOutBox>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        <SectionHeader icon={Database} title="أهمية CMMS في إدارة الأصول" />
        <h4 className="text-md font-bold text-slate-700 mb-3">قاعدة بيانات مركزية للأصول</h4>
        <DataTable columns={assetDataCols} rows={assetDataRows} />
        <CallOutBox type="tip" title="تتبع تاريخ الأصل">
          يوفر النظام سجلا كاملا لكل أصل يشمل جميع الأعطال وأعمال الصيانة وقطع الغيار المستخدمة والتكاليف والتعديلات الفنية، مما يمكن من اتخاذ قرارات مدروسة بشأن الإصلاح أو الاستبدال.
        </CallOutBox>
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">تحسين قرارات الإصلاح والاستبدال</h4>
        <FlowChart
          nodes={[
            ...
            { id: "كفاءة", label: "كفاءة التشغيل", icon: "📊" },
          ...
          edges={[
            ...
            { from: "معايير", to: "عمر" }, { from: "معايير", to: "كفاءة" },
            { from: "أعطال", to: "قرار" }, { from: "تكلفة", to: "قرار" },
            { from: "عمر", to: "قرار" }, { from: "كفاءة", to: "قرار" },
            { from: "قرار", to: "إصلاح" }, { from: "قرار", to: "استبدال" },
          ]}
        />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">إدارة العقود والضمانات</h4>
        <DataTable columns={warrantyCols} rows={warrantyRows} />
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        <SectionHeader icon={Smartphone} title="تطبيق CMMS الجوال" />
        <h4 className="text-md font-bold text-slate-700 mb-3">مميزات التطبيق الجوال</h4>
        <DataTable columns={mobileAppCols} rows={mobileAppRows} />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">آلية العمل بدون إنترنت (Offline Mode)</h4>
        <FlowChart
          dir="LR"
          nodes={[
            ...
            { id: "حفظ", label: "حفظ محلياً", icon: "💾" },
            { id: "استعادة", label: "مزامنة تلقائية عند العودة", icon: "🔄", color: "green" },
          ]}
          edges={[
            { from: "فني", to: "اتصال" },
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        ...
        <h4 className="text-md font-bold text-slate-700 mb-3">مصادر البيانات الحالية</h4>
        <DataTable columns={migrationCols} rows={migrationRows} />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">خطة الترحيل على 5 مراحل</h4>
        <DataTable columns={migrationPlanCols} rows={migrationPlanRows} />
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mt-4 space-y-2">
          <p className="text-sm font-bold text-slate-700">قواعد الترحيل الأساسية:</p>
          <ol className="text-sm text-slate-600 space-y-1 list-decimal list-inside">
            <li><strong>لا نرحّل بيانات غير موثوقة</strong> — البيانات الخاطئة أسوأ من عدم وجود بيانات</li>
            <li><strong>ندخل الأساسيات أولاً</strong> — الأصول + المستخدمون + البلاغات الجارية، ثم التفاصيل</li>
            <li><strong>التحقق بعد الترحيل</strong> — مراجعة 10% من البيانات المدخلة للتأكد من دقتها</li>
            <li><strong>فترة انتقالية</strong> — العمل بالنظام القديم والجديد بالتوازي لمدة أسبوعين</li>
          </ol>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        ...
        <h4 className="text-md font-bold text-slate-700 mb-3">تصنيف المخاطر الأمنية</h4>
        <DataTable columns={riskCols} rows={riskRows} />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">إجراءات الأمان في CMMS</h4>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        ...
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">خطة الاستمرارية التشغيلية</h4>
        <FlowChart
          nodes={[
            { id: "عطل", label: "عطل في النظام", icon: "🚨", color: "red" },
            { id: "تصنيف", label: "نوع العطل", icon: "❓", color: "amber" },
            { id: "بسيط", label: "خلل بسيط (بطء أو خطأ)", icon: "⚠️" },
            { id: "كلي", label: "تعطل كامل (سيرفر أو DB)", icon: "🚫", color: "red" },
            { id: "إبلاغ", label: "إبلاغ فريق الدعم" },
            { id: "إصلاح_بسيط", label: "إصلاح خلال ساعتين", icon: "✅", color: "green" },
            { id: "تفعيل", label: "تفعيل خطة الطوارئ", icon: "📋" },
            { id: "يدوي", label: "تشغيل النماذج الورقية", icon: "📝" },
            { id: "استعادة", label: "استعادة النظام من النسخة الاحتياطية", icon: "🔄" },
            { id: "اختبار", label: "اختبار النظام المستعاد", icon: "🔍" },
            { id: "إيقاف", label: "إيقاف الخطة اليدوية", icon: "⏹️" },
            { id: "تعويض", label: "إدخال البلاغات الورقية بأثر رجعي", icon: "📥" },
          ]}
          edges={[
            { from: "عطل", to: "تصنيف" },
            { from: "تصنيف", to: "بسيط" }, { from: "تصنيف", to: "كلي" },
            { from: "بسيط", to: "إبلاغ" }, { from: "إبلاغ", to: "إصلاح_بسيط" },
            { from: "كلي", to: "تفعيل" },
            { from: "تفعيل", to: "يدوي" },
            { from: "يدوي", to: "استعادة" },
            { from: "استعادة", to: "اختبار" },
            { from: "اختبار", to: "إيقاف" },
            { from: "إيقاف", to: "تعويض" },
          ]}
        />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">الحقيبة الورقية للطوارئ</h4>
        <DataTable columns={kitCols} rows={kitRows} />
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-6 print:p-4 shadow-sm">
        <SectionHeader icon={Monitor} title="تكامل CMMS مع أنظمة المباني (BMS/BAS)" />
        <h4 className="text-md font-bold text-slate-700 mb-3">أنواع التكامل الممكنة</h4>
        <DataTable columns={bmsCols} rows={bmsRows} />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">سيناريو التكامل</h4>
        <FlowChart
          dir="LR"
          nodes={[
          ...
          edges={[
            ...
            { from: "cmms", to: "مدير", label: "تنبيه بعطل محتمل" },
            { from: "مدير", to: "فني", label: "توزيع المهمة" },
            { from: "فني", to: "cmms", label: "إغلاق بعد الإصلاح" },
          ]}
        />
        <h4 className="text-md font-bold text-slate-700 mt-5 mb-3">خطة التكامل التدريجي</h4>
